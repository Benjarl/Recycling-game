...
namespace LabData
{
    ...
    public class LabDataManager : MonoSingleton<LabDataManager>
    {
        ...
        public void DataCollect(IData data, bool loop = true, int frequency = 200)
        {
            ...
            LabTools.CreatSaveDataFolder(basePathStr);
            ...
            basePathStr = LabTools.CreatSaveDataFolder(basePathStr);

            if (_saveType == SaveType.Csv)
            {
                data.SaveDataList.ForEach(p =>
                {
                    string dataPath = string.Join("_", basePathStr + "/" + DateTime.Now.ToString(_localSaveDataTimeLayout), userStr, p.GetType().Name, p.Invoke() + "." + _saveType.ToString());
                    LabTools.CreatData(dataPath);
                    DataWriter dw = new DataWriter(dataPath, p.Invoke, _saveType);
                    dw.WriteCsvTitle();
                    dw.Dispose();
                });
            }
